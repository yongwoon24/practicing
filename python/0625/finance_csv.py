...
with open('stock_data.csv', 'w', encoding='utf-8-sig', newline='') as f:
    wr = csv.writer(f)
    wr.writerow(['종목명', '현재가', '전일비', '등락률', '거래량', '거래대금', '매수호가', '매도호가', '시가총액', 'PER', 'ROE'])
    ...
    for row in rows:
            ...
            if len(cells) >= 11:
                ...
                wr.writerow([item, current_price, yday, change_rate, trade_amount, trade_price, buy_price, sell_price, total_price, per, roe])
