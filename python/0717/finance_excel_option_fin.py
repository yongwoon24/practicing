...
def get_user_selection(prompt, valid_options):
    while True:
...
def scrape_data(select_theme, select_kind):
    options = Options()
...
# 사용자 입력
select_theme = get_user_selection('1:거래량 / 2:상승 / 3:하락 / 4:시가총액  >> ', ['1', '2', '3', '4'])
select_kind = get_user_selection('1:코스피 / 2:코스닥 >> ', ['1', '2'])

select_theme_array = ['거래량', '상승', '하락', '시가총액']
data = scrape_data(select_theme, select_kind)

df = pd.DataFrame(data, columns=['종목명', '현재가', '전일비', '등락률', '거래량', '거래대금', '매수호가', '매도호가', '시가총액', 'PER', 'ROE'])

# Excel 파일명 생성
name = '코스피' if select_kind == '1' else '코스닥'
date = datetime.now().strftime("%Y%m%d")
excel_filename = f"{date}_{name}_종목_{select_theme_array[int(select_theme)-1]}순_정렬.xlsx"
